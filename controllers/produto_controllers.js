...
exports.novo = [
  body("nome").trim().notEmpty().withMessage("O Nome tem que ser especificado"),
  body("valor_normal")
    .notEmpty()
    .withMessage("Valor normal nao especificado")
    .isNumeric()
    .withMessage("São aceitos apenas números"),
  body("valor_reduzido").isNumeric().withMessage("São aceitos apenas números"),
  asyncHandler(async (req, res) => {
    ...
    if (hasIt) {
      res.status(400).json({ message: "Produto já registrado" });
    } else if (!err.isEmpty()) {
